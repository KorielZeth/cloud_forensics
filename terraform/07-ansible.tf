...
# Provision the lab using Ansible from the attackbox machine
resource "null_resource" "ansible-provisioning" {
  ...
  triggers = {
    ...
    dc_id = azurerm_windows_virtual_machine.ForLab-vm-dc.id
    windows10_id = azurerm_windows_virtual_machine.ForLab-vm-windows10.id
    attackbox_id = azurerm_linux_virtual_machine.ForLab-vm-attackbox.id
    wazuh_id = azurerm_linux_virtual_machine.ForLab-vm-wazuh.id
  }

  connection {
    type  = "ssh"
    host  = azurerm_public_ip.ForLab-ip.ip_address
    user  = var.linux-user
    password = random_string.linuxpass.result
  }

  # Copy Ansible folder to attackbox machine for provisioning
  provisioner "file" {
    source      = "../Ansible"
    destination = "/dev/shm"
  }
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "cd /dev/shm/Ansible",
      "ansible-playbook -v cloud_for.yml"
    ]
  }
}
